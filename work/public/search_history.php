...
if (count($err) > NUMBER_OF_ERRORS) {
  ...
  if (isset($_SESSION['send_parts'])) {
    $send_parts = $_SESSION['send_parts'];
    $search_history = array_merge($login_user, $send_parts);
  }
  $history_count = OrderLogic::searchHistry($login_user, $search_history);
}
...
<div class="container">
    <?php if(!empty($history_err)) : ?>
      <h3 class="title-center">購入検索履歴</h3>
        <table class="table">
          <thead>
            <tr>
              <th class="col">No</th>
    ...
    <?php else : ?>
      <h3 class="title-center">注文検索履歴</h3>
      <div class="container">
        <div class="d-flex flex-row-reverse">
